...
const wawatiny = (opts) => {
  options = opts
  continueNum = opts.retryTimes || 10
  limitNum = opts.limit || 5
  ...
  return {
    ...
    task: async (ctx, task) => {
      ...
      const filesPromise = files.map(item => {
        const outputPath = options.output ? item.replace(options.input, options.out) : item
        return limit(() => {
